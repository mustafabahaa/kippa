...
> The CLI is also available as a standalone binary from [Firebase Tools releases](https://github.com/firebase/firebase-tools/releases).
...
**Windows:** download and run the installer from the [Google Cloud CLI installation guide](https://cloud.google.com/sdk/docs/install).
...
Create a project in the [Firebase Console](https://console.firebase.google.com), then run `firebase use --add <PROJECT_ID>` from the repo root.
...
Vite starts on [localhost:5173](http://localhost:5173). The app will:
